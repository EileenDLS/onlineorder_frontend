export const login = (credentials) => {
  ...
  return fetch(loginUrl, {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw Error("Fail to log in");  // throw exception, so if who use login function, who will catch this exception
    }
  }); // returns a promise
};
...
export const signup = (data) => {
  ...
  return fetch(signupUrl, {
    ...
    // convert a JavaScript object (data) into a JSON string representation
...
export const getMenus = (restId) => {
  return fetch(`/restaurant/${restId}/menu`).then((response) => {
    ...
    // reads the response body and parses it as JSON, returns a promise that resolves with the result of parsing the body text as JSON
